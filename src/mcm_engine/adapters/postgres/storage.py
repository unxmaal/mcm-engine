...
_DDL_STATEMENTS: list[str] = [
    ...
    # v13: source_classification (issue #105) on all four entity tables for
    # EXISTING postgres deployments. Additive nullable free-form label the
    # engine carries but never interprets. Not in any tsv generated column, so
    # no tsv rebuild. Per-table guards keep it idempotent.
    """
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                       WHERE table_name = 'knowledge' AND column_name = 'source_classification') THEN
            ALTER TABLE knowledge ADD COLUMN source_classification TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                       WHERE table_name = 'negative_knowledge' AND column_name = 'source_classification') THEN
            ALTER TABLE negative_knowledge ADD COLUMN source_classification TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                       WHERE table_name = 'errors' AND column_name = 'source_classification') THEN
            ALTER TABLE errors ADD COLUMN source_classification TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    ...
    # #103: recall_log generalized from knowledge-only to all four entity
    # types. Add entity_type to EXISTING recall_log tables; the default
    # backfills historical (knowledge) recalls. Idempotent catalog read.
    """
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                       WHERE table_name = 'recall_log' AND column_name = 'entity_type') THEN
            ALTER TABLE recall_log ADD COLUMN entity_type TEXT NOT NULL DEFAULT 'knowledge';
        END IF;
    END$$
    """,
    ...
    def insert_error(self, row: ErrorRow) -> int:
        with self._conn.cursor() as cur:
            if row.id:
                cur.execute(
                    "INSERT INTO errors "
                    "(id, pattern, context, root_cause, fix, tags, project, source_classification) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",
                    (row.id, row.pattern, row.context, row.root_cause, row.fix,
                     row.tags, row.project, row.source_classification),
                )
            else:
                cur.execute(
                    "INSERT INTO errors "
                    "(pattern, context, root_cause, fix, tags, project, source_classification) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id",
    ...
    def page_entries(
        ...
        with _borrow(self) as conn:
            with conn.cursor() as cur:
                ...
                return [hydrate(r) for r in cur.fetchall()]
